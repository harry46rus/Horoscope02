...
def syd():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'\bсуд[иеаоуы]|\bосуд[ия]|присуди|\bпристав\bсуд\b|осужден'
        # 'ся' учитывается и добавляется
...
def svo():
    ...
    for title, y in json_data_news.items():
        ...
        |(границ\ц*)[\s\w]{,30}(белгородс|курской)|украин|диверс[иа]|диверси|\bz[а-я]*|ььь\
        |госизмен|шпион|террористическ\w* опасност|подозрительн\w* лиц|мобилизаци|ььь\
        |минобороны|контрактник|служб\w* по контракту'
        # 'ся' учитывается и добавляется
