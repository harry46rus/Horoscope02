...
def anons():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'буд[еу]т|произойд[уе]т|установ[ия]т|начн[уе]т|создадут|представ[ия]т\
...
def accidents():
    ...
    for title, y in json_data_news.items():
        ...
        # 'ся' учитывается и добавляется
        key_list1 = r' дтп|произош[е]*л\w*|случил[о]с*\w*|убийств\w*|подозрева\wт\w*\
                   ...
                   | смерт\w\w |обманул\w*|мошенни[кч]|пресекли|обезвредили|поймал|выманил\
                   ...
                   | наркокурьер\w*|горит|\bвор[аеуо]|клиент\w* банк|суд обязал\
                   ...
                   | угнан| укус\w* клещ|\bподлог|\bфиктивн|(чужой|не своей)[\s\w]{,15}(картой)\
                   |\bподлог|\bзадержа|\bподделал|разыскива|полици|ранени|\bскончал|тушил|погиб\
                   |возгорани|гибел|покушени|изнасилова|план\w* \W?перехват\
                   |прос[я]т сообщ[и]т|потушил|врезал[и]?с\
                   |дорожн\w* ситуац|сводк\w* чс'
        # key_list1 = ['[а-я]+[аеиюя]т\s',]#'[а-я]+ят ','[а-я]+ют ','[а-я]+ат ']
...
def societ():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r' построен| запущен| завершен\w*| преоборудован\w*| открыт\w*\
                    ...
                    | в собственность регион|итоги работы|эскроу-счет|футбол[оауе]|собственник\
                    ...
                    | победил| безработиц| инфляц| рост\w* цен| физкультур|запустил|депутат\
                    | археолог|индекс\w* потребительских цен|одержал\w [\S*\s]* победу\
                    | проходит| добил\w*с|муниципал| слушани|совещан|мэр[иаоеу]|гимн\
...
def jkh():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'жкх|без газа|гасоснабж|канализац|отключен|отключат\
                    ...
                    |(отключени\w*|без )[\s\w]{,15}(вод[^и]|свет|электр|движени\w*)\
                    |дорожн\w* развязк\w*|капремонт|капитальн\w* ремонт\
                    |(вод|свет|электр)[\s\w]{,15}(отключени\w*)|«Квадр[аыо]\w*»\
                    |(холодн|горяч)[\s\w]{,15}(вод)| мусор| свалк|\sтбо\
                    |подач\w газ\w| инвестированн\w|освещенн\w? улиц|теплосет\w*\
                    |(вод|свет|электр)[\s\w]{,15}(обещают)| дорожн\w* работ| нацпроект\
                    |(ремонт|укладк|строит|убирал|уборк)[\s\w]{,15}(доро[гж])|утилизаци\w* тко\
...
def sport():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 =r'чемпионат|авангард|бокс|маунтинбайк|\s?спорт\
                ...
                |многоборь|(завоевал)[\s\w]{,15}(золото|серебр|бронз)|олимпиад\
...
def medicin():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 =r'поликлиник|больниц|амбулатори|скор\w* помощ\
                ...
                |уролог|(делал|выполнил|провел)[\s\w]{,15}(операц)|врач\
...
def education():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 =r'педагог|школ|детск\w* сад|детсад\
                ...
                |грамот|академи|стипендиат|грамотност|\bit|\
...
def economic():
    ...
    for title, y in economic_dict.items():
        key_list1 = r'мошенник|похити[лтв]|укра[вл]|лиши[вл]|потеря[вл]|отня[втл]|обману[тлв]\
                    ...
                    |отсудил|смерт|ущерб|(чужой|не своей)[\s\w]{,15}(картой)\
                    |\bподлог|\bзадержа|\bподделал|в суде|судебн\w* иск'
...
def culture():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'конкурс|фестивал|концерт|джаз\
                    |экскурси|\bкино|\bактер|пев[ие]ц|\bпоэт\
                    |композитор|культур[еауо]|праздни[кч]|\bпоказ\
                    |фильм|лауреат|премьер[аоы]|спектакл|выставка|\bпати\
                    |им\.\sпушкин|винцкевич|party|библиотек|филармрони\
                    |режисер|по радио|культурн\w*[-\s\w]{,10}мероприят|галере\w* «ая»\
                    |оркестр|солист|вернисаж|мемориал|туристическ\w* маршрут\
                    |(создадут|установ[ия]т|постав[ия]т)[\s\w]{,15}(памятник|доск)\
                    |благотворительн фонд|капелл|вечер\w* памяти|\bпьес[ыаоу]\
                    |художни[кц]|хореографическ|искусств|стипендиат|живопис\
                    |театр|\bтенор|xxi век|литературн|анимаци|\bмузе[йяея]|музыкальн\
                    |мастер-класс|пленэр|пушкинск\w* карт|художествен|мастериц\
                    |торжественн\w* открыти|ден\w* город|(творческ\w*)(встреч|вечер)\
                    |нов[ы]\w* год|традиционн\w* ремес|водян\w* мельниц|комеди[яеи]\
