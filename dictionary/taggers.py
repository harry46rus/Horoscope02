...
def anons():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'буд[еу]т|произойд[уе]т|установ[ия]т|начн[уе]т|создадут|представ[ия]т|ььь\
          ...
          | обеспеч[аи]т|огранич[аи]т|установ[яи]т|обяж[уе]т|познаком[яи]т|слёт|ььь\
...
def jkh():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'жкх|без газа|гасоснабж|канализац|отключен|отключат|ььь\
                    ...
                    |(коммунальн\w*) (удобств|услуг)|\bотоплени'
...
def medicin():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 =r'поликлиник|больниц|амбулатори|скор\w* помощ|ььь\
                ...
                |\bомс\b|медсестр|здравоохранени|\bрод[ыо]|пандеми|ььь\
                ...
                |\bорви |планов\w* помощ|госпитал|\bклиник'
...
def fire():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'пожар|возгорани|тушил|воспламен[еи]|[с]?горел|\bдым|очаг\w? возгорания'
        # 'ся' учитывается и добавляется
...
def syd():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = r'\bсуд[иеаоуы]|\bосуд[ия]|присуди|\bпристав\bсуд\b|осужден|подсудим'
        # 'ся' учитывается и добавляется
...
def svo():
    ...
    for title, y in json_data_news.items():
        ...
        |военн\w* комиссар|(фейк\w*|слух\w*)[\s\w]{,30}(эвакуац)|военком|призыв'
        # 'ся' учитывается и добавляется
...
# =====================================================
    ...
    def transft(dd,mon,yy):
        ...
        sec = int(time.mktime((yy, mon, dd, 1, 0, 0, 0, 0, 0)))
