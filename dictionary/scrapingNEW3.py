...
def anons():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = 'буд[еу]т|произойд[уе]т|установ[ия]т|начн[уе]т|создадут|представ[ия]т| \
                    наград[яи]т|анонс\w?|приглашают|провед[еу]т|восстановлен|\
                ...
                отправ[ия]т|пройд[еу]т|поддержат|решил|\
...
def accidents():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = 'дтп\s|произош[е]?л\w?|случил[о]с?\w?|убийств\w?|подозрева\wт\w?|покушени\w?|\
                    ...
                    риговори\w?|тюрьм\w?|за решетку|сбил\w?|стрел\w?|горел\w?|пожар|\
                    смерт\w\w |обманул\w?|мошенник\w|пресекли|обезвредили|поймал|\
...
def societ():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = ' построен| запущен| завершен\w?| преоборудован\w?| открыт\w?|\
                     ...
                     остановлен\w?| восстановле\w?| отремонтирован\w?| возобновлен\w?|\
                     ...
                     потрачен\w|вложен\w| инвестированн\w| заплати\w?| вруч\w?|льгот|\
