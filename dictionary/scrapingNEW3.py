...
def anons():
    ...
    for title, y in json_data_news.items():
        #
        key_list = [f'\d+ {get_day_month(1)[1]}', f'[вна] {get_day_month(1)[1]}',
                    f'\d+ {get_day_month(2)[1]}', f'[вна] {get_day_month(2)[1]}',
                    f'\d+ {get_day_month(3)[1]}', f'[вна] {get_day_month(3)[1]}',
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = 'буд[еу]т|произойд[уе]т|установ[ия]т|начн[уе]т|создадут|представ[ия]т| \
                ...
                перекро[юе]т|приглас[ия]т|отмет[ия]т|встрет[ия]т|распредел[ия]т|\
                благоустро[ия]т|восстанов[ия]т|появ[ия]тся|стро[ия]тся|постро[ия]т|\
                отправ[ия]т|пройд[еу]т|\
                собира[ею]тся|обеспеч[аи]т|откро[ею]тся|замен[ия]т|заверш[аи]тся|получ[аи]т|\
                ожида[ею]т|объяв[ия]т|оцен[ия]т'
...
def accidents():
    ...
    for title, y in json_data_news.items():
        ...
        key_list1 = 'дтп\s|произош[е]?л\w?|случил[о]с?\w?|убийств\w?|подозрева\wт\w?|покушени\w?| \
                    ...
                    столкновени\w?|поврежд\w?|перевернул\w?| пострада\w?| жертв\w?\
                    риговори\w?|тюрьм\w?|за решетку|сбил\w?|стрел\w?|горел\w?|пожар\w?|\
                    смерт\w\w |\
                    ...
                    беспилотн\w?|обнаружил\w?|по горячим следам|совращени\w?|\
                    ...
                    снаряд\w?|выписал\w?|протокол\w?|несанкционированн\w?|торговл\w?|наркокурьер\w?|\
...
time_list0 = ['00:00', '00:20', '00:40', '01:55', '02:07',
